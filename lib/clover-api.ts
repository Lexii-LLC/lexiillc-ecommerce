...
async function fetchCloverInventoryPage(
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON response from Clover API: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`
    )
  }
...
/**
 * Direct server-side fetch function to get Clover inventory
 */
  ...
  try {
    ...
    while (pageCount < maxPages) {
      ...
      try {
        ...
        if (pageCount === 0) {
          console.warn(
            'Initial paginated request failed, trying without pagination parameters'
          )
          try {
            const items = await fetchCloverInventoryPage(
              baseUrl,
              CLOVER_API_TOKEN
            )
    ...
    if (pageCount >= maxPages) {
      console.warn(
        `Reached maximum page limit (${maxPages}). Some items may be missing.`
      )
    }

    console.log(
      `Fetched ${allItems.length} items from Clover API across ${pageCount + 1} page(s)`
    )
    return allItems
  } catch (error) {
